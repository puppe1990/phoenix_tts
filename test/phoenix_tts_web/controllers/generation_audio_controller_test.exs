defmodule PhoenixTtsWeb.GenerationAudioControllerTest do
  ...
  setup do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "phoenix-tts-generation-audio-#{System.unique_integer([:positive])}"
      )
  ...
  test "streams embedded generation audio inline", %{conn: conn} do
    generation =
      %Generation{}
    ...
    assert get_resp_header(conn, "content-disposition") == [
             "inline; filename=\"generation-#{generation.id}.mp3\""
           ]
  end

  test "serves embedded generation audio as attachment when download is requested", %{conn: conn} do
    generation =
      %Generation{}
      |> Generation.persistence_changeset(%{
  ...
  test "falls back to legacy local files when the row predates embedded audio", %{conn: conn} do
  ...
  test "redirects back with flash when generation audio is missing", %{conn: conn} do
    conn = get(conn, ~p"/generations/999999/audio")

    assert redirected_to(conn) == ~p"/recentes"
    assert Phoenix.Flash.get(conn.assigns.flash, :error) == "Áudio não encontrado."
  end
end
